...
const PartsSearchTool = () => {
  ...
  const searchForMatchingParts = (searchTerm) => {
    ...
    const suggestions = partsData.filter(part => {
      return terms.every(term => {
        let fieldsToSearch;
        if (matchingSearchType === 'description') {
          fieldsToSearch = [part.description1, part.description2];
        } else {
          fieldsToSearch = [part.eurolinkItem, part.vendorItem, part.description1, part.description2];
        }
        ...
        return fieldsToSearch.some(field => {
          ...
          if (/^\d+$/.test(term)) {
            return fieldLower.includes(term) || fieldLower.includes(term + '-');
          } else {
  ...
  const addMatchToTariffSheet = async (unmatchedPartNumber, matchedPart) => {
    ...
    const updatedUploadResults = {
      ...uploadResults,
      data: uploadData,
      matched: uploadResults.matched + rowsUpdated,
